...
const baseProductSchema = Joi.object({
  name: Joi.string().min(3).max(255).trim().messages({
  ...
  category: Joi.string().min(2).max(100).trim().messages({
...
export const updateProductSchema = baseProductSchema
  .fork(['name', 'description', 'price', 'stock', 'category'], (schema) => schema.optional())
  .min(1)
  .messages({
